...
namespace UniHR {
    ...
    public class Combiner : IHttpHandler, IRequiresSessionState {
        ...
        public void ProcessRequest(HttpContext context) {
            ...
            // Read setName, contentType and version. All are required. They are used as cache key
            string setName = request["s"] ?? string.Empty;
            string contentType = request["t"] ?? string.Empty;
            string version = request["v"] ?? string.Empty;
            string sFileName = request["f"] ?? string.Empty;

            // Decide if browser supports compressed response
            bool isCompressed = DO_GZIP && this.CanGZip(context.Request);

            // Response is written as UTF8 encoding. If you are using languages like Arabic, you should change this to proper encoding 
            ...
            if (!this.WriteFromCache(context, setName, version, isCompressed, contentType)) {
                using (MemoryStream memoryStream = new MemoryStream(5000)) {
                    // Decide regular stream or GZipStream based on whether the response can be cached or not
                    using (Stream writer = isCompressed ? (Stream)(new GZipStream(memoryStream, CompressionMode.Compress)) : memoryStream) {

                        // Load the files from the session and process each file
                        ...
                        string sSessionName = setName + "_" + sFileName.ToUpper();
                        string setDefinition = string.Empty;
                        if (HttpContext.Current.Session[sSessionName] != null) {
                            setDefinition = HttpContext.Current.Session[sSessionName].ToString() ?? "";
                        ...
                        foreach (string fileName in fileNames) {
                            ...
                            writer.WriteByte(Convert.ToByte('\r'));
        ...
        private byte[] GetFileBytes(HttpContext context, string virtualPath, Encoding encoding) {
            ...
            } else if (virtualPath.Contains("UniHR.")) {
        ...
        /// <summary>
        /// Converts a value from stream to a byte array
        /// If value is null or invalid it returns null.
        /// </summary>
        /// <param name="PsStream"></param>
        /// <returns>Byte array from the stream</returns>    
        public static byte[] StreamToByteArray(Stream PsStream) {
            try {
                byte[] outByte = new byte[PsStream.Length];
                // Read the input stream for its entire length
                PsStream.Read(outByte, 0, outByte.Length);
                return outByte;
            } catch (Exception) {
                return null;
            }
        ...
        private string GetCacheKey(string setName, string version, bool isCompressed) {
            return "UniHR." + setName + "." + version + "." + isCompressed;
        }
